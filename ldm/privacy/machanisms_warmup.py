...
# def gdp_mech(sample_rate3, niter3, alpha_cumprod_W, sample_rate1, sample_rate2, niter1, niter2, sigma,
#              alpha_cumprod_S, d, delta):
#
#     mu_3 = sample_rate3 * sqrt(niter3 * (np.exp(4 * d * alpha_cumprod_W / (1 - alpha_cumprod_W)) - 1))   # warm-up phase
#
...
#
#     mu = sqrt(mu_3 ** 2 + mu_2 ** 2)
#     epsilon = eps_from_mu(mu, delta)
#     return epsilon

def gdp_mech(sigma_s, sample_rate1, niter1, alpha_cumprod_W, sample_rate2, niter2, sigma, alpha_cumprod_S, d, delta):
...
def eps_from_config(config):
    ...
    alpha_cumprod_W = alphas_cumprod[config.model.params.timesteps - 1].numpy()
    ...
    epsilon = gdp_mech(
        ...
        sample_rate1=prob1,
        niter1=niter1,
        alpha_cumprod_W=alpha_cumprod_W,
        sample_rate2=prob2,
        niter2=niter2,
