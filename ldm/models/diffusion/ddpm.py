...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.noise_scale_s  = dp_config.noise_scale_s
        self.n_classes = cond_stage_config.params.n_classes
    ...
    # frozen VAE
    def instantiate_first_stage(self, config, client_index=None):
        ...
        if client_index is not None:
            ckpt_map = getattr(config, "client_ckpt", {})
            ckpt_path = ckpt_map.get(str(client_index)) or ckpt_map.get(int(client_index))
            state = torch.load(ckpt_path, map_location='cuda:0')
            sd = state.get("state_dict", state)
            # sd = {k.replace("first_stage_model.", "").replace("module.", ""): v for k, v in sd.items()}
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # Adaptive computing of scaling factors for different datasets
        # self.all_latents.append(z.cpu())
        # all_latents_tensor = torch.cat(self.all_latents)
        # std = all_latents_tensor.std().item()
        # normalizer = 1 / std
        # print(f'{normalizer = }')

        # print("latent size min:", z.min().item(), "max:", z.max().item())
        ...
        if self.model.conditioning_key is not None:
            ...
            if bs is not None:
                ...
                if self.model.conditioning_key == "crossattn":
                    ...
                    c_subset = {k: v[:bs] for k, v in c.items()}
                    c = c_subset
    ...
    def shared_step(self, batch, phase, **kwargs):
        x, c = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, phase=phase)
        return loss

    def forward(self, x, c, phase, *args, **kwargs):

        if phase == "1":

            t = torch.randint(self.num_timesteps, (x.shape[0],), device=self.device).long()

            x = torch.clamp(x, min=-1, max=1.0)  # clip the latent to [-1, 1]
            # add extra Guassian noise
            x = x + torch.randn_like(x) * self.noise_scale_s
            ...
            if self.model.conditioning_key is not None:
                ...
                if self.cond_stage_trainable:
                    # phase1: condition=10 (unconditional)
                    c = {k: self.n_classes * torch.ones_like(v) for k, v in c.items()}
            ...
            x_repeated = x_repeated.reshape(
                x.shape[0] * self.num_noise_sample,
                x.shape[1],
                x.shape[2],
                x.shape[3],
            )
            ...
            # Noise multiplicity
            t = torch.randint(self.num_timesteps, (x.shape[0] * self.num_noise_sample,), device=self.device).long()

            if c is not None:
                # classifier-free guidance
                # c = self.apply_cfg_to_condition(c, p=0.1, n_classes=self.n_classes)

                # Noise multiplicity
                c_repeated = {}
                for k, v in c.items():
                    v = v.unsqueeze(1).repeat_interleave(self.num_noise_sample, dim=1)
                    v = v.reshape(x.shape[0] * self.num_noise_sample, *v.shape[2:])
                    c_repeated[k] = v
                c = c_repeated
            ...
            # # Noise multiplicity
            # if c is not None:
            #     c_repeated = c.unsqueeze(1).repeat_interleave(
            #         self.num_noise_sample,
            #         dim=1,
            #     )
            #
            #     c_repeated = c_repeated.reshape(
            #         x.shape[0] * self.num_noise_sample,
            #         c.shape[1],
            #         c.shape[2],
            #     )
    ...
    def apply_cfg_to_condition(self, c, p=0.1, n_classes=10):
        with torch.no_grad():
            ...
            if y is None:
                raise ValueError("The input must contain the cond_stage_key.")

            boolean_ = torch.bernoulli(p * torch.ones_like(y, device=self.device)).bool()
            no_class_label = n_classes * torch.ones_like(y, device=self.device)
            y = torch.where(boolean_, no_class_label, y)
            c[self.cond_stage_key] = y
        return c
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.model.conditioning_key == 'adm':
                ...
                # for cross-attention based conditional model
    ...
    def configure_optimizers(self):
        ...
        # 5. optimizer for Phase I
        if self.phase == "1":

            # 6. Check if optimizer is already initialized
            if hasattr(self, 'optimizer') and self.optimizer is not None:
                return self.optimizer

            # co-training with conditional embedding
            if self.cond_stage_trainable:
                params = list(self.model.parameters()) + list(self.cond_stage_model.parameters())
                opt = torch.optim.AdamW(params, lr=self.learning_rate1)
            else:
                opt = torch.optim.AdamW(self.model.parameters(), lr=self.learning_rate1)

            # opt = torch.optim.AdamW(self.model.parameters(), lr=self.learning_rate1)

        # 1. optimizer for two phases
        # elif self.phase == "1":
        #
        #     # 2. Check if optimizer is already initialized
        #     if hasattr(self, 'optimizer') and self.optimizer is not None:
        #         return self.optimizer
        #     opt = torch.optim.AdamW(self.model.parameters(), lr=self.learning_rate1)
            ...
            # 3. Check if DP optimizer is already initialized
            if hasattr(self, 'dp_optimizer_initialized') and self.dp_optimizer_initialized:
                return self.optimizer

            # co-training with conditional embedding
            if self.cond_stage_trainable:
                params = list(self.model.parameters()) + list(self.cond_stage_model.parameters())
                opt = torch.optim.AdamW(params, lr=self.learning_rate2)
            else:
                opt = torch.optim.AdamW(self.model.parameters(), lr=self.learning_rate2)
            ...
            if self.dp_config and self.dp_config.enabled:
                ...
                _, opt, _ = self.privacy_engine.make_private(
                    ...
                    noise_multiplicity=self.dp_config.num_noise_sample,
                )

                self.dp_optimizer_initialized = True
