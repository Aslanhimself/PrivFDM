...
def train_all_classifiers(dataset_names, synthetic_train_loader, synthetic_test_loader, device, batch_size):
    if  dataset_names == 'MNISTTrain':
        ...
        real_test_dataset = torchvision.datasets.MNIST(
            ...
            transform=torchvision.transforms.Compose([torchvision.transforms.Resize(32), torchvision.transforms.ToTensor()]))
            # transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))
        real_train_loader = torch.utils.data.DataLoader(dataset=real_train_dataset, shuffle=False, batch_size=batch_size)
        real_test_loader = torch.utils.data.DataLoader(dataset=real_test_dataset, shuffle=False, batch_size=batch_size)

    elif dataset_names == 'FashionMNISTTrain':
        real_train_dataset = torchvision.datasets.FashionMNIST(
            root='data/fashionmnist/', train=True, download=True,
            transform=torchvision.transforms.Compose([torchvision.transforms.Resize(32), torchvision.transforms.ToTensor()]))
            # transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))
        real_test_dataset = torchvision.datasets.FashionMNIST(
    ...
    elif dataset_names == 'CIFAR10Train':
        real_train_dataset = torchvision.datasets.CIFAR10(root='data/CIFAR10/', train=True, download=True,
            transform=torchvision.transforms.ToTensor())
        real_test_dataset = torchvision.datasets.CIFAR10(root='data/CIFAR10/', train=False, download=True,
            transform=torchvision.transforms.ToTensor())
        real_train_loader = torch.utils.data.DataLoader(dataset=real_train_dataset, shuffle=False,batch_size=batch_size)
        real_test_loader = torch.utils.data.DataLoader(dataset=real_test_dataset, shuffle=False, batch_size=batch_size)

    train_cnn_acc, test_cnn_acc = train_cnn(
        synthetic_train_loader, synthetic_test_loader, real_train_loader, real_test_loader, device)
    ...
    return train_cnn_acc, test_cnn_acc
...
def main():
    ...
    dataset_names = get_dataset_names(config.data)
    ...
    with torch.no_grad():
        ...
        for class_label in classes:
            ...
            # for cross-attention label embeding
            ...
            for idx in range(iter):
                ...
                samples_ddim, _ = sampler.sample(S=ddim_steps,
                                                 ...
                                                 conditioning={model.cond_stage_key: torch.full((batch_size,), class_label, device=model.device, dtype=torch.long)},
                                                 ...
                                                 # unconditional_conditioning={model.cond_stage_key: torch.full((batch_size,), n_classes, device=model.device, dtype=torch.long)},
    ...
    _, test_cnn_acc = train_all_classifiers(dataset_names, train_queue, eval_queue, device, 128)
